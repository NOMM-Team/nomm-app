...
class FomodSelectionDialog(Adw.Window):
    ...
    def populate_listbox(self, list_box, options):
        ...
        for name, desc, source, flags in options:
            ...
            plugin_type = check_for_plugin_dependencies(self.module_data, self.game_dest, self.current_step,
                                                        self.current_group, plugin_index, self.active_flags)
        ...
        if (selection_type == 'SelectAtMostOne' or selection_type == 'SelectExactlyOne') and not is_not_usable:
